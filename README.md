...
盔甲架只画**架体本身**：身上的**装备（盔甲/手持物）不渲染**（那需要原版实体贴图与装备层模型，不在方块图集里），
`Pose` 姿态也按直立处理，只保留朝向与 `Small` / `ShowArms` / `NoBasePlate` 三个开关。
...
- **实体内置贴图**：展示框与盔甲架的贴图都由工具链内置（`voxelith:frame/empty`、`voxelith:entity/armor_stand`），
  不依赖资源包里的贴图名，所以任何版本、任何资源包都不会让它们变成品红。
- **多遍渲染的共享图集**：分遍渲染会先打一张共享图集供各批复用，而地图画、空框、实体内置贴图是**运行时注册**的——
  它们会被显式并进预打包清单；否则单遍正常、分遍变紫（这条曾是真实故障）。
