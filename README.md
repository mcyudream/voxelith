...
#   -PmaxChunks=<上限>   窗口超过就拒绝（护栏，避免整图级别窗口把进程拖死）
# 注意：切批要复用一张预打好的共享图集，因此**只在有采集产物时生效**
#（缺 models.json.gz 时自动退回单遍——单遍同样正确，只是内存与窗口成正比）。
...
| 盔甲架 | 读实体 NBT 取坐标、`Rotation[0]` 朝向与 `Small` / `ShowArms` / `NoBasePlate` 开关，按原版比例摆出**简化盒体**（高 2 方块：底座 12×1×12、双腿 2×11×2、躯干 4×12×2、手臂 2×12×2 仅在 `ShowArms` 时画、头 8×8×8；`Small` 整体 ×0.5）。贴图是**内置木纹**（`voxelith:entity/armor_stand`），不查资源包，所以换版本、换资源包都能画出来。同一条实体来源同时读 `entities/` 与区块 NBT，并按维度定位 |
| 盔甲架的装备 | 四格**盔甲**按原版盔甲盒体（玩家部件尺寸 + 膨胀，外层 1.0px / 护腿 0.5px）包在架身上，贴图用**真实的盔甲层贴图**（先试 1.21.2+ 的 `textures/entity/equipment/humanoid[_leggings]/`，回退旧版 `textures/models/armor/<材质>_layer_1/2`；64×32 的层贴图自动补成正方形进图集，避免拉伸），UV 按标准盒体展开——铁甲看得出铁甲、钻石甲看得出钻石甲。**手持物**：生成型物品（剑/工具/食物…）画成手位的 16×16 精灵片，方块类物品画 8px 小立方（取方块贴图）；`ShowArms=false` 的架子不持物。物品 NBT 兼容 1.20.5+ 的 `equipment{}` 与旧版 `ArmorItems`/`HandItems`；缺贴图的装备按件跳过并在日志里计数 |

> 注入的实体几何**只进 3D 瓦片**，不进 LOD 的地表高度场/航拍色（原版高度场同样只统计方块）——
> 否则盔甲架的底座顶面会被当成地表，把高度抬到实体所在的 y，空中装饰用的盔甲架还会在 LOD 里拉出细柱。

**不会渲染**：画（painting）、掉落物、船/矿车、生物、玩家等**任何其它实体**——它们不在方块数据里。
盔甲架装备的已知取舍：染色盔甲不取 NBT 里的染色（按底图绘制）、鞘翅与雕刻南瓜等特殊头胸物品不画，
`Pose` 姿态按直立处理，只保留朝向与 `Small` / `ShowArms` / `NoBasePlate` 三个开关。
...
- **实体存放与物品 NBT**：Paper / 原版 1.20.2+ 的 `entities/r.X.Z.mca` 与更早的区块 `entities` 列表都读；
  地图编号兼容 1.20.5+ 数据组件与旧 `tag.map`；下界/末地按 `DIM-1`/`DIM1` 定位。展示框与盔甲架共用**同一份实体读取实现**，
  不会出现「一种实体能读、另一种读不到」的分叉。
- **实体内置贴图**：展示框与盔甲架本体的贴图由工具链内置（`voxelith:frame/empty`、`voxelith:entity/armor_stand`），
  不依赖资源包里的贴图名；盔甲/手持物则用资源包里的**真实贴图**（新旧两代盔甲贴图布局都认），
  缺贴图的装备按件跳过并计数，不会把整张图渲染打断。
- **多遍渲染的共享图集**：分遍渲染会先打一张共享图集供各批复用，而地图画、空框、实体内置贴图与
  盔甲/手持物贴图是**运行时注册/解析**的——它们会被显式并进预打包清单；否则单遍正常、分遍变紫（这条曾是真实故障）。
