...
| Phase 6 | 规模化与增量：十万级区块压测、region 监听增量更新、S3 存储 | 🚧 进行中（管线状态机 + 断点续跑 + region 分片 + WatchService 增量 + FILE/S3 SPI + 可选量化 + IncrementalRenderPort 真实 bake→tile→lod + 图集落盘/新瓦片插入/全图高度场合并；meshopt 熵编码未接。十万级全量烘焙仍走 jshell） |
