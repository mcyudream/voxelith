...
public class GenerateTilesUseCase {
    ...
    public TileOutcome generate(TileCommand command) {
        ...
        byte[] atlasPng = imageCodec.encodePng(atlas.pixelSize(), atlas.pixelSize(), atlas.argb());
        Path atlasFile = sink.writeAtlas(command.outputDir(), atlasPng);
        ...
        Path reportFile = sink.writeReport(command.outputDir(), summaries,
                atlas.layout().cellIndex().size(), atlas.pixelSize());
        return new TileOutcome(summaries, atlasFile, reportFile,
                atlas.layout().cellIndex().size(), atlas.pixelSize());
    }
